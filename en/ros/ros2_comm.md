...
The PX4/ROS 2 build process ([see below](#ros-2-ros-applications)) decouples generation of the ROS 2 IDL files from building PX4.
[px4_msgs](https://github.com/PX4/px4_msgs) contains the UORB definitions from PX4 firmware (automatically added by CI) and builds ROS 2 compatible IDL messages.
The IDL messages generated by `px4_msgs` are used by the code generators in `px4_ros_com` to create the agent source code.
