...
The Auterion [VIO bridge ROS node](https://github.com/Auterion/VIO_bridge) provides a bridge between the camera and ROS.
...
- Connect the [T265 Intel Realsense Tracking Camera](../peripherals/camera_t265_vio.md) using the supplied USB-C to USB-A connector.
- Mount the camera with lenses pointing down if at possible (default).
- The camera is very senstive to vibration; a soft mounting for vibration isolation is recommended.
...
- On the companion computer, install and configure [MAVROS](https://dev.px4.io/master/en/ros/mavros_installation.html).
  Verify it is connected successfully to the flight controller.
  > **Tip** You can use the *QGroundControl* [MAVLink Inspector](https://docs.qgroundcontrol.com/en/analyze_view/mavlink_inspector.html) to verify that you're getting `HEARTBEAT` messages from the companion computer.
- Get the Auterion [VIO bridge ROS node](https://github.com/Auterion/VIO_bridge):
  ...
  - Build the package:
	...
	```
- Configure the *VIO bridge launch file* (typically [bridge_mavros.launch](https://github.com/Auterion/VIO/blob/master/launch/bridge_mavros.launch) or [bridge.launch](https://github.com/Auterion/VIO/blob/master/launch/bridge.launch) ) to reflect your camera orientation.
  No configuration is required if the lenses face down (the default).
- Follow the instructions [below](#ekf2_tuning) for tuning the EKF2.


To run VIO, call `roslaunch` with an appropriate launch file:
...
Most commonly you'll use `bridge_mavros.launch` as shown:
```
cd ~/catkin_ws/src
...
In reality, this may need some empirical tuning becuase delays in the communication chain are very setup-specific.
