...
1. First set up *rsync* (this is is used to transfer files from the development computer to the target board over a network - WiFi or Ethernet). 
   For *rsync* over SSH with key authentication, follow steps here (on the development machine):
   1. Generate an ssh key if you have not done that in the past already: `ssh-keygen -t rsa`
     1. ENTER //optionally enter a passphrase
     1. ENTER
     1. ENTER
   1. Define the BeagleBone Blue board as `beaglebone` in **/etc/hosts**
   1. `ssh-copy-id root@beaglebone`. Alternatively you can use the beaglebone's IP: `ssh-copy-id root@<IP>`
...
1. Cross Compile Setup
   1. Toolchain download
      ...
      1. Add it to the PATH in ~/.profile as shown below
         ...
         ```
         > **Note** Logout and Login to make the change working, or execute the same line on your current shell.
