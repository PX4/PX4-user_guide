...
The v1.15 release brings lots of new upgrades and fixes thanks to the tremendous response from 1.14 and the addition of the uXRCE-DDS interface. This release brings tons of quality of life changes to developers and integrators using PX4 as a target trough the uXRCE-DDS interface, from networking to middleware improvements, all around making this an ideal release for anyone already enjoying the many benefits of 1.14.

Additionally we are bringing new hardware support from sensors to drivers, thanks to our growing network of partners from across the industry. In this release you will enjoy updated support for common hardware options, in part sponsored by the manufacturer themselves. We are thankful for their support.

Lastly, we are bringing new improvements to navigation which should result in a more enjoyable experience and straightforward debugging. We hope more users try this release and report any issues to the developer team on GitHub. ([Learn how to raise an issue here](https://docs.px4.io/main/en/contribute/support.html#issue-bug-reporting)).
...
- The **jMAVSim** simulator is no longer the default simulation engine for PX4-Autopilot, we are now moving to using **Gazebo** as the default simulation solution, please consider upgrading as we will be focusing our efforts in Gazebo.
- The `SYS_MC_EST_GROUP` parameter has been removed, and there are now dedicated parameters. Continue reading for a description of the changes.
...
- Derivation changed to **Error-State Kalman Filter** [PX4-Autopilo#22262](https://github.com/PX4/PX4-Autopilot/pull/22262)
...
- Use Joseph stabilized covariance update algorithm for better covariance stability and allow use of "consider states" (inactive states with non-zero variance). [PX4-Autopilo#22770](https://github.com/PX4/PX4-Autopilot/pull/22770)
