...
If the vehicle won't arm, you can more easily find out why in the QGC Arming Checks UI.
...
As part of this change, it is now possible to switch to any mode when disarmed (previously could not switch to a mode that required GPS if you didn't have a good position estimate).
PX4 will only allow you to arm when the conditions for the current mode are met, and will report only on the failure checks that are relevant to the current mode.
...
We updated the ROS 2 interface, replacing [Fast-RTPS](https://docs.px4.io/v1.13/en/middleware/micrortps.html) with [uXRCE-DDS](../ros/ros2_comm.md), resulting in an improved experience across the board.
...
1. Default disarmed PWM was changed from 900us to 1000us.
...
- [DroneCAN sensors now need to be individually enabled via `UAVCAN_SUB_*` parameters](../dronecan/README.md#dronecan-sensor-subscriptions) - [PX4-Autopilot#18471](https://github.com/PX4/PX4-Autopilot/pull/18471)
