...
<a id="VT_QC_ALT_LOSS"></a>[VT_QC_ALT_LOSS](../advanced_config/parameter_reference.md#VT_QC_ALT_LOSS) | Uncommanded descent quad-chute altitude threshold. An uncommanded descend quad-chute event will be triggered as soon as the vehicle drops below this value measured relative to the current altitude reference. The altitude reference is computed based on the vehicle altitude and the altitude setpoint as follows. If the vehicle flies above the current altitude sepoint, then the altitude reference is set to the current altitude setpoint. No event will be triggered as the altitude error is positive. If the vehicle flies below the current altitude setpoint, then the altitude reference is set to the maximum altitude the vehicle has achieved while flying below the current altitude setpoint. This is to make sure that an uncommanded quad-chute event is not triggered in case the altitude setpoint increases faster than the vehicle can follow. On the other hand an event will still be triggered if the vehicle continuously loses altitude.
<a id="VT_QC_T_ALT_LOSS"></a>[VT_QC_T_ALT_LOSS](../advanced_config/parameter_reference.md#VT_QC_T_ALT_LOSS) | Altitude loss threshold for quad-chute triggering during VTOL transition to fixed-wing flight. Quad-chute triggers if the current altitude is more than this value below the altitude at the beginning of the transition.
