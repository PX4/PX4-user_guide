...
--- | --- | ---
<a id="COM_RC_LOSS_T"></a>[COM_RC_LOSS_T](../advanced_config/parameter_reference.md#COM_RC_LOSS_T) | Manual Control Loss Timeout | Time after last setpoint received from the selected manual control source. This must be kept short because the vehicle will continue to fly using the old manual control setpoint until the timeout triggers.
