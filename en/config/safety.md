...
During flight, the failure detector can be used to trigger [flight termination](../advanced_config/flight_termination.md) (in all modes) if failure conditions are met, and can then launch a [parachute](../peripherals/parachute.md) or perform some other action.
...
Note that this is always enabled on takeoff, irrespective of the `CBRK_FLIGHTTERM` parameter.
