...
- You may need to modify the [COM_FAIL_ACT_T](#COM_FAIL_ACT_T) parameter.
...
If triggered, the vehicle will immediately switch to multicopter mode and execute action defined in parameter [COM_QC_ACT](#COM_QC_ACT) .
...
<a id="VT_QC_HR_ERROR_I"></a>[VT_QC_ALT_LOSS](../advanced_config/parameter_reference.md#VT_QC_ALT_LOSS) | Uncommanded descent quad-chute altitude threshold. An uncommanded descend quad-chute event will be triggered as soon as the vehicle drops below this value measured realtive to the current altitude reference. The altitude reference is computed based on the vehicle altitude and the altitude setpoint as follows. If the vehicle flies above the current altitude sepoint, then the altitude reference is set to the current altitude setpoint. No event will be triggerd as the altitude error is positive. If the vehicle flies below the current altitude setpoint, then the altitude reference is set to the maximum altitude the vehicle has achieved while flying below the current altitude setpoint. This is to make sure that an uncommanded quad-chute event is not triggered in case the altitude setpoint increases faster than the vehicle can follow. On the other hand an event will still be triggered if the vehicle continuously loses altitude.
