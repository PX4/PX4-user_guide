...
-	All cells are relatively small so larger packs are made up of many cells tied in series and parrel to create the required voltage and capacity
-	Lower discharge rates relative to battery size (C rating)
-	More difficult to adapt to high current usage vehicles
-	Lower charging rates (relative to capacity)
-	Requires more stringent temperature monitoring during charge and discharge
-	Requires settings changes on the ESC to utilize max capacity given “standard” ESC low voltage settings are too high.
-	At close to empty battery pack, the volage of the battery is such that a ~3v difference (while using a 6S battery) is possible from a Lipo to Li-ion which could have implications on thrust expectations. 


C Ratings:

-	A C rating is simply a multiple of the stated capacity of any battery type. A C rating is relevant (and differs) for both charge and discharge rates.
    •	For example, a 2000 mAh battery (irrespective of voltage) with a 10C discharge rate would mean that, that battery can safely and continuously discharge 20 amps of current (2000/1000=2Ah x 10C = 20 amps).
-	C Ratings are always given by the manufacturer and often on the outside of the battery pack. While they can actually be calculated, you need several pieces of information and to measure the internal resistance of the cells.
...
-	Following manufacturer guidelines for both charge and discharge C ratings is very important for the health of your battery and to operate your vehicle safely as well as to reduce fires, “puffing” packs and other suboptimal states during charging and discharging.


Energy Density:

-	Energy density is how much energy is able to be stored relative to its weight. It is generally measured and compared in watt hour per kilogram (Wh/KG).
...
-	This number 185 Wh/Kg would be on the very high-end for a lipo battery. A li-ion battery on the other hand can reach 260 Wh/Kg, meaning per KG of battery onboard you can carry 75 more watt-hours. 
...
-	Lipo batteries are easily available and can withstand higher discharge rates that are common in multi-rotor aircraft.
