...
The check is only enabled when the rangefinder is not used as the primary height source, and is only active while the vehicle is hovering (as it assumes a static ground height).
