...
The first field that has a non-zero value (from top to bottom) defines what valid estimate is required in order to use offboard mode, the setpoint message(s) that can be used, and the controllers that are enabled.
For example, if the `acceleration` field is the first non-zero value, then PX4 requires a valid `velocity estimate`, the setpoint must be specified using the `TrajectorySetpoint` message `acceleration` field, and the position and velocity controllers are disabled (so that even if set, the setpoint position and velocity field will not be used).
...
|--------------------------|:--------------:|:--------------:|:------------------:|:--------------:|:---------------:|:---------------:|:-----------------:|---------------------------------------------------------------------------------------------------------------------------------|
| position (NED)           |     &check;    |        -       |          -         |        -       |        -        |        -        |      position     | `TrajectorySetpoint.position`                                                                                                              |
| velocity (NED)           |     &cross;    |     &check;    |          -         |        -       |        -        |        -        |      velocity     | `TrajectorySetpoint.velocity`                                                                                                              |
| acceleration (NED)       |     &cross;    |     &cross;    |       &check;      |        -       |        -        |        -        |      velocity     | `TrajectorySetpoint.acceleration`                                                                                                              |
...
:::note
Generally you should only use the setting message and fields corresponding to the bit set in `OffboardControlMode`.
If you set a field that injects an offboard setpoint at a lower level than the highest active controller, it may conflict with setpoints generated by a higher controller.
:::
