...
[PWM ESCs](../peripherals/pwm_escs_and_servo.md) are commonly used for fixed wing vehicles and ground vehicles (vehicles that require a higher response rate like multicopters typically use oneshot or dshot ESCs).
...
A higher rate is better, in particular where a rapid response to setpoint changes is needed.
...
Unlike [dshot](#dshot) and [UAVCAN ESC](#oneshot-125) they do not have the ability to provide telemetry and feedback on ESC (or servo) state.
...
PX4 configuration is done by setting the desired rate .
The parameter name indicates the data rate in kbaud - e.g. DShot150 has a rate of 150kbaud.
Generally higher rates are better, but lower rates may be needed for larger aircraft (with longer leads).
...
[UAVCAN ESCs](../peripherals/uavcan_escs.md) are highly recommended when UAVCAN is the primary bus used for your vehicle.
