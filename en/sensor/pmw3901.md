...
- The distance sensors should be enabled as described in their setup documentation (no flow-specific configuration is required).
  For sensors connected by SPI or I2C (but not UAVCAN), [SENS_EN_PMW3901](../advanced_config/parameter_reference.md#SENS_EN_PMW3901) must be set `1` in order to enable the sensor driver.
