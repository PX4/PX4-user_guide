...
It is a tracking sensor similar to what you would find in a computer mouse but adapted to work between 80 mm and infinity.
...
- Outdoor flight: ≥10 meters (e.g. to support the position control in environments where GPS might have issues)
...
Flow modules are typically mounted near the center of the vehicle.
...
The diagram shows the Bitcraze board's orientation to the vehicle that corresponds to `SENS_FLOW_ROT=0` (note the notch at the back).
...
- The distance sensors should be enabled as described in their setup documentation (no flow-specific configuration is required).
  For sensors connected by SPI or I2C (but not UAVCAN/UART), [SENS_EN_PMW3901](../advanced_config/parameter_reference.md#SENS_EN_PMW3901) must be set `1` in order to enable the sensor driver.
- For UART-connected PMW3901-based flow sensors (e.g. ThoneFlow-3901UY) you must also [configure the associated serial port](../peripherals/serial_configuration.md).
  This is done by setting the parameter [SENS_TFLOW_CFG](../advanced_config/parameter_reference.md#SENS_TFLOW_CFG) to the value of the UART port you connected.
  For example, if you connected this sensor to `TELEM 2`, you need to set `SENS_TFLOW_CFG` to `102`.
...
The sensor range is minimal (2 meters) and will be reduced when flying in the sunlight.
...
An SPI cable has seven wires, from which you need to connect 6 to the flow board.
...
The board also incorporates a distance sensor (We recommend you use an [external distance sensor](#external_rangefinder)).
...
The Hex [HereFlow PMW3901 Optical Flow Sensor](http://www.proficnc.com/all-products/185-pixhawk2-suite.html) is a tiny board containing the PMW3901 flow module, VL53L1X distance sensor, and an IMU to synchronize the flow gyro data better.
