...
:::note
페이로드(액추에이터)는 [사전시동 상태](../getting_started/px4_basic_concepts.html#arming-and-disarming)에서 테스트할 수 있으며, 이 상태에서는 모터를 비활성화되며 액츄에이터는 동작 가능합니다. 이 방법은 차량이 시동 상태에서 테스트하는 것보다 더 안전합니다.
:::
...
다음 항목에서는 카메라를 *연결* 방법과 PX4 설정 방법을 설명합니다.
