...
:::note
부하 보상을 사용하려면 [기본 구성](#basic_settings)을 설정하여야 합니다. :::caution 이 방법은 추력 명령과 전류 사이에 지연이 있고 추력이 전류에 선형 적으로 비례하지 않기 때문에 정확하지 않습니다.
:::
...
:::
1. 기본 설정 화면에서 [전압 분배기당 전류값](#current_divider)을 보정해야합니다.
...
:::note
이것은 상대적 배터리 소모량을 측정하는 가장 정확한 방법입니다.
부팅시마다 새로 충전한 배터리를 정확하게 설정하면 추정 품질이 스마트 배터리의 품질과 비슷해질 것입니다 (이론적으로 정확한 남은 비행 시간 추정이 가능합니다).
:::
...
1. 매개 변수 [BAT1_CAPACITY](../advanced_config/parameter_reference.md#BAT1_CAPACITY)를 라벨에 인쇄된 배터리 용량의 약 90 %로 설정합니다.
