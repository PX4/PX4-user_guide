...
- 수직 위치와 후방 위치 사이의 값은 `min=-90` 및 `max=0`입니다. 즉, 한 그룹의 모든 출력은 동일한 프로토콜에서 동일한 속도로 작동하여야 합니다(예: 한 그룹의 모든 출력에 대해 400Hz의 PWM 신호).
...
PWM 출력은 하드웨어 타이머 그룹을 기반으로 그룹화됩니다. 아래 슬라이더 예는 일반적인 VTOL Tiltrotor 기체에 대한 섹션을 나타냅니다.
...
1. 무장 해제 시 표면이 중립 위치를 유지하도록 `비시동 해제` 값을 설정합니다. This is usually around `1500` for PWM servos (near the centre of the servo range).
...
3. Move the slider again to the middle and check if the Control Surfaces are aligned in the neutral position of the wing.
   ...
   - If it is not aligned, you can set the **Trim** value for the control surface.
     ...
     ::: info This is done in the `Trim` setting of the Geometry panel, usually by "trial and error". ![Control Surface Trimming](../../assets/config/actuators/control_surface_trim.png)
:::
