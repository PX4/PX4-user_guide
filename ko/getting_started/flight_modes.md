...
비행 모드는 자동조종장치가 무선 조종기에 반응 메커니즘과 자율비행 방법을 정의합니다.
...
마지막으로, 멀티콥터 [자율 모드](#categories)에서 RC 스틱을 움직이면 차량이 [위치 모드](../flight_modes/position_mc.md)로 [기본적으로](../advanced_config/parameter_reference.md#COM_RC_OVERRIDE) 변경됩니다(중요한 배터리 안전 장치를 처리하지 않는 한). 고정익 비행에서는 스틱 이동이 무시됩니다.
