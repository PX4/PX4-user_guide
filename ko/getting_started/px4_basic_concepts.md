...
[데이터/텔레메트리 라디오](../telemetry/README.md)은 *QGroundControl*과 같은 지상 통제국과 PX4를 구동하는 기체를 MAVLink로 무선 연결합니다. 비행중인 기체의 매개변수 값 조정, 실시간으로 텔레메트로 통신, 비행 중 임무 변경 등의 작업을 수행합니다.
...
기체에는 전원을 공급하게 되면 안전 사고를 유발하는 모터와 프로펠러와 같은 부품들이 있습니다.

사고 발생을 줄이는 방법들은 다음과 같습니다.

- 비행중이 아닐 때는 PX4의 *시동을 해제* (전원 차단)하고, 이륙 전에 *시동*을 켜는 것이 좋습니다.
- 기체가 빠른 시간 안에 이륙하지 않으면, 착륙후의 기체는 자동으로 시동이 해제됩니다. 시동 해제 시간은 매개변수로 설정할 수 있습니다.
- 일부 기체에는 시동전에 조작하는 [안전 스위치](#safety_switch)(보통 GPS 수신기의 일부임)가 장착되어 있습니다.
- 기체가 정상 상태가 아니면, 시동은 걸리지 않습니다.
- 수직이착륙기는 고정익 모드로 설정하면, 시동이 걸리지 않습니다([기본 설정](../advanced_config/parameter_reference.md#CBRK_VTOLARMING)).

모드 2 수신기의 경우에는 추진력/방위각 조절 스틱을 *우측 하단*에 1초 정도 위치하면 시동이 걸립니다. 시동을 해제하려면 좌측 하단으로 둡니다. PX4에서 무선 조종 스위치로 시동을 걸도록 설정할 수 있습니다. 또한, MAVLink로 시동 명령을 지상통제국에서 전송할 수도 있습니다.
...
비행 모드는 다양한 유형과 수준의 차량 자동화 기능을 제공합니다. *자율 모드*는 자동조종장치에 의해 완전히 자동으로 제어되며, 원격으로 수동 제어할 필요가 없습니다. 예를 들어, 이륙, 원 위치 복귀 및 착륙하는 과정 등의 작업을 자동화합니다. 또 다른 자율 모드에는 사전에 프로그래밍 된 임무를 수행하거나, GPS 위치 정보 비행이나, 외부 컴퓨터 또는 지상관제국 명령에 의한 비행 조종이 가능합니다.

*수동 모드*는 비행 콘트롤러를 사용자가 RC로 직접 제어합니다. 다른 수동 모드는 다른 특징을 가능 비행을 할 수 있습니다. 예를 들어, 곡예 비행이나 바람이 불어도 일정환 경로나 위치를 유지하는 비행을 할 수 있습니다.

:::tip
모든 기체마다 모든 비행 모드를 이용할 수 없으며, 일부 모드는 특정 조건이 충족된 기체에서만 이용할 수 있습니다. 대부분의 모드에서는 GPS 위치 정보가 필요합니다.
:::

비행 모드에 대한 개요는 [비행 모드](../getting_started/flight_modes.md)를 참고하십시오. 원격 스위치로 비행 모드를 변경하는 방법은 [비행 모드 설정](../config/flight_mode.md)을 참고하십시오.

<a id="safety"></a>

## 안전 설정(사고 방지) 

PX4는 시스템의 사고를 대비하여 기체을 보호하고 복구할 수 있는 안전 시스템이 있으며, 이와 관련된 각종 설정들이 있습니다! 이를 통하여 안전 비행 지역과 조건을 설정하고, 안전 장치가 작동(예: 착륙, 위치 유지 또는 지정된 지점으로 복귀)시에 수행하는 작업을 설정할 수 있습니다.
...
차량, 보트 및 항공기에는 전진하는 방향이 있습니다.

![프레임 방향](../../assets/concepts/frame_heading.png)

:::note VTOL
테일 시터의 방향은 멀티콥터 설정(즉, 이륙, 호버링, 착륙 중 차량 포즈)에 의해 결정됩니다.
:::

비행 콘트롤러를 차량의 이동 벡터와 정렬시키기 위해서는 기체의 전진 방향을 아는 것이 중요합니다. 멀티콥터는 모든 방향에서 대칭인 경우에도 전진 방향이 정의됩니다. 제조사에서는 일반적으로 프로펠러나 팔(프레임)에 색깔을 사용하여 차량의 전진 방향을 표시합니다.
