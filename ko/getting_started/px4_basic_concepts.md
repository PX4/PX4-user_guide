...
비행 모드는 다양한 유형과 수준의 차량 자동화 기능을 제공합니다. *자율 모드*에서는 자동조종장치에 의해 자동으로 제어되며, 원격으로 수동 제어하지 않아도 됩니다. 예를 들어 이륙, 복귀 및 착륙 등의 작업을 자동으로 제어할 수 있습니다. 또 다른 자율 모드에는 사전 프로그래밍된 임무 수행, GPS 위치 정보 비행, 외부 컴퓨터 또는 지상제어S/W 명령에 의한 비행 작업이 가능합니다.
