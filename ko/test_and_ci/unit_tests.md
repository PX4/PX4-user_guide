...
1. [Google Test](https://github.com/google/googletest/blob/master/docs/primer.md)를 사용한 단위 테스트("GTest") - 최소한의 내부 전용 종속성이 있는 테스트
...
`make tests`를 실행하여 시험을 진행할 수 있으며, 이 과정 후 `build/px4_sitl_test/functional-MyNewFunctional` 위치에서 바이너리를 찾을 수 있습니다. 디버거에서 직접 실행할 수 있지만 [--gtest_filter=\<regex\>](https://github.com/google/googletest/blob/master/docs/advanced.md#running-a-subset-of-the-tests) 인수를 사용하여 실행 가능 호출당 하나의 테스트만 실행하도록 주의하십시오. uORB 및 매개변수 라이브러리의 일부는 자체적으로 완벽하게 정리되지 않고 여러 번 설정하면 정의되지 않은 동작이 발생할 수 있습니다.
