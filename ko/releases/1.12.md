...
* **부팅 안정성** LSE(RTC) 발진기가 시작되도록 보장하는 V5-V6X
  ...
  * [**BACKPORT**] stm32h7: LSE 기능을 자동 선택하는 옵션 추가
    ...
    :::note
이 노브는 낮은 값에서 높은 값으로 올바른 값을 순환합니다. 크리스탈 손상을 방지하기 위하여, OSC를 실행하는 가장 낮은 설정을 사용하려고합니다. 애플리케이션 노트 AN2867 참조
실리콘의 회전 수를 고려하고 올바른 코드 포인트를 사용하여 드라이브 강도를 얻습니다. Eratta ES0392 Rev 7 2.2.14 참조
