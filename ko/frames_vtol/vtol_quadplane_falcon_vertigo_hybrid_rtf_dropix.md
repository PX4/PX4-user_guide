...
2. 자동 조종 장치가 차량 앞쪽을 기준으로 [옆으로](#flight_controller_orientation) 장착되므로 [자동조종장치 방향 ](../config/flight_controller_orientation.md)을 `ROTATION_YAW_270`으로 설정합니다. 나침반은 앞쪽을 향하므로 기본값 (`ROTATION_NONE`)을 사용합니다.
