...
"Мізками" дрону є автопілот. Він складається з програмного забезпечення _політного стека_ яке виконується на _контролері апарата_ ("політному контролері").

Деякі безпілотники також мають окремий бортовий [companion комп'ютер](#offboard-companion-computer). Він надає потужну обчислювальну платформу для машинного зору, мережевих та багатьох інших задач.
...
- Великий вибір hardware для [контролерів рухомих засобів](#vehicle-flight-controller-board), датчиків та іншої периферії.
- Гнучкі та потужні [режими польоту](#flight-modes) та [функції безпеки](#safety-settings-failsafe).
- Надійна та глибока інтеграція з [companion комп'ютерами](#offboard-companion-computer) та [robotics API](../robotics/README.md) ([ROS 2](../ros/ros2_comm.md), [MAVSDK](http://mavsdk.mavlink.io)).
