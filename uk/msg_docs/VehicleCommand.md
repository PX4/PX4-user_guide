...
Vehicle Command uORB повідомлення. Використовується для управління місією / дією / тощо. Дотримується визначення MAVLink COMMAND_INT / COMMAND_LONG

[вихідний файл](https://github.com/PX4/PX4-Autopilot/blob/release/1.15/msg/VehicleCommand.msg)
...
uint16 VEHICLE_CMD_NAV_WAYPOINT = 16            # Navigate to MISSION. |Тримати час в десяткових секундах. (ігнорується фіксованим крилом, час залишатися на MISSION за обертовим криттям) | Радіус прийняття в метрах (якщо сфера з цим радіусом влучає, MISSION вважає як досягнуто)| 0, щоб пройти через WP, якщо радіус > 0 в метрах, щоб передати WP. Позитивне значення для обертання за годинниковою стрілкою, від'ємне значення для обертання проти годинникової стрілки. Дозволяє керування траєкторією.| Бажаний кут крену на політному МІСТЕРІ (вертольот)| Широта| Довгота| Висота|
uint16 VEHICLE_CMD_NAV_LOITER_UNLIM = 17        # Обертання навколо цього МІСТЕРА необмежену кількість часу |Порожній| Порожній| Радіус навколо МІСТЕРА, в метрах. If positive loiter clockwise, else counter-clockwise| Desired yaw angle.| Latitude| Longitude| Altitude|
...
uint16 VEHICLE_CMD_DO_ORBIT = 34            # Start orbiting on the circumference of a circle defined by the parameters. |Радіус [м] |Швидкість [м/с] |Поведінка курсу |Порожній |Широта/X |Довгота/Y |Висота/Z |
uint16 VEHICLE_CMD_DO_FIGUREEIGHT = 35              # Почати польот по контурі восьмірки, визначеної параметрами. |Великий радіус [м] |Малий радіус [м] |Швидкість [м/с] |Орієнтація |Широта/X |Довгота/Y |Висота/Z |
uint16 VEHICLE_CMD_NAV_ROI = 80             # Встановлює область інтересу (ROI) для набору сенсорів або самого транспортного засобу. Це потім може бути використано системою керування транспортними засобами для керування ставленням транспортного засобу та ставленням різних датчиків, таких як камери. |Регіону інтересу. (see MAV_ROI enum)| MISSION index/ target ID. (see MAV_ROI enum)| ROI index (allows a vehicle to manage multiple ROI's)| Empty| x the location of the fixed ROI (see MAV_FRAME)| y| z|
...
uint16 VEHICLE_CMD_NAV_GUIDED_LIMITS = 90       # set limits for external control |timeout - maximum time (in seconds) that external controller will be allowed to control vehicle. 0 означає відсутність обмеження часу| абсолютна мінімальна висота (у метрах, AMSL) - якщо транспортний засіб опуститься нижче цієї висоти, команда буде припинена, а місія продовжиться.  0 means no lower altitude limit| absolute altitude max (in meters)- if vehicle moves above this alt, the command will be aborted and the mission will continue.  0 means no upper altitude limit| horizontal move limit (in meters, AMSL) - if vehicle moves more than this distance from it's location at the moment the command was executed, the command will be aborted and the mission will continue. 0 means no horizontal altitude limit| Empty| Empty| Empty|
...
uint16 VEHICLE_CMD_DO_SET_PARAMETER = 180       # Set a system parameter.  Caution!  Використання цієї команди вимагає знання числового значення переліку параметра. |Parameter number| Parameter value| Empty| Empty| Empty| Empty| Empty|
...
uint16 VEHICLE_CMD_DO_LAND_START = 189          # Mission command to perform a landing. Це використовується як маркер у місії, щоб повідомити автопілоту, де починається послідовність елементів місії, яка представляє посадку. Це також може бути відправлено через COMMAND_LONG для виклику посадки, у цьому випадку буде використана найближча (географічно) послідовність посадки в місії. Широта / Довгота є необов'язковою і може бути встановлена на 0/0, якщо не потрібно. Якщо вказано, то воно буде використовуватися для пошуку найближчої послідовності посадки. |Empty| Empty| Empty| Empty| Latitude| Longitude| Empty|
...
uint16 VEHICLE_CMD_DO_SET_ROI_LOCATION = 195        # Sets the region of interest (ROI) to a location. Це потім може бути використано системою керування транспортними засобами для керування ставленням транспортного засобу та ставленням різних датчиків, таких як камери. |Empty| Empty| Empty| Empty| Latitude| Longitude| Altitude|
...
uint16 VEHICLE_CMD_DO_SET_ROI_NONE = 197        # Cancels any previous ROI command returning the vehicle/sensors to default flight characteristics. Це потім може бути використано системою керування транспортними засобами для керування ставленням транспортного засобу та ставленням різних датчиків, таких як камери. |Empty| Empty| Empty| Empty| Empty| Empty| Empty|
...
uint16 VEHICLE_CMD_DO_SET_ROI = 201         # Sets the region of interest (ROI) for a sensor set or the vehicle itself. Це може бути використано системою керування транспортними засобами для керування позицією транспортного засобу та позицією різноманітних датчиків, таких як камери. |Region of interest mode. (see MAV_ROI enum)| MISSION index/ target ID. (see MAV_ROI enum)| ROI index (allows a vehicle to manage multiple ROI's)| Empty| x the location of the fixed ROI (see MAV_FRAME)| y| z|
...
uint16 VEHICLE_CMD_PREFLIGHT_SET_SENSOR_OFFSETS = 242   # Set sensor offsets. Ця команда буде прийнята лише у режимі передпольотної підготовки. |Sensor to adjust the offsets for: 0: gyros, 1: accelerometer, 2: magnetometer, 3: barometer, 4: optical flow| X axis offset (or generic dimension 1), in the sensor's raw units| Y axis offset (or generic dimension 2), in the sensor's raw units| Z axis offset (or generic dimension 3), in the sensor's raw units| Generic dimension 4, in the sensor's raw units| Generic dimension 5, in the sensor's raw units| Generic dimension 6, in the sensor's raw units|
uint16 VEHICLE_CMD_PREFLIGHT_UAVCAN = 243       # UAVCAN configuration. Якщо param 1 == 1, мапування приводу та призначення напрямку повинні бути запущені
uint16 VEHICLE_CMD_PREFLIGHT_STORAGE = 245      # Запит на збереження різних значень параметрів та журналів. Ця команда буде прийнята лише у режимі передпольотної підготовки. |Parameter storage: 0: READ FROM FLASH/EEPROM, 1: WRITE CURRENT TO FLASH/EEPROM| Mission storage: 0: READ FROM FLASH/EEPROM, 1: WRITE CURRENT TO FLASH/EEPROM| Reserved| Reserved| Empty| Empty| Empty|
...
uint16 VEHICLE_CMD_FIXED_MAG_CAL_YAW = 42006            # Magnetometer calibration based on provided known yaw. Це дозволяє швидку калібрування за допомогою таблиць поля WMM в автомобілі, враховуючи лише відомий кут кочення автомобіля. Якщо широта і довгота обидва дорівнюють нулю, тоді використовуйте поточне місцезнаходження транспортного засобу.
