...
uint16 VEHICLE_CMD_NAV_WAYPOINT = 16            # Navigate to MISSION. |Hold time in decimal seconds. (ignored by fixed wing, time to stay at MISSION for rotary wing)| Acceptance radius in meters (if the sphere with this radius is hit, the MISSION counts as reached)| 0 to pass through the WP, if > 0 radius in meters to pass by WP. Positive value for clockwise orbit, negative value for counter-clockwise orbit. Дозволяє керування траєкторією.| Бажаний кут крену на політному МІСТЕРІ (вертольот)| Широта| Довгота| Висота|
