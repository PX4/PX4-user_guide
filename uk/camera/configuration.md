...
PX4 також може бути налаштований використовувати [бекенд інтерфейсу спрацювання MAVLink](#trigger-interface-backends) з камерою MAVLink.
...
Камери MAVLink рекомендовано, оскільки камери, підключені безпосередньо, підтримують лише [невеликий набір](#mavlink-command-interface-directly-connected-cameras) доступних повідомлень та команд камери MAVLink.
Камери MAVLink, ймовірно, пропонують набагато більше контролю над камерою за допомогою [Протоколу камери MAVLink](https://mavlink.io/en/services/camera.html).
