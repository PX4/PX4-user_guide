...
# PX4 Автопілот керівництво користувача
...
PX4 це _Professional Autopilot_. Розроблено розробниками світового класу з промисловості та наукових закладів і підтримується активною спільнотою у світі він блокує всілякі транспортні засоби від гонки та вантажних дронів на сухопутні автомобілі та батисками.
...
[Початок роботи](getting_started/README.md) цей гайд мають прочитати всі користувачі! Включає в себе огляд PX4, функції, які надає стек польотів (режими польоту та функції безпеки) та підтримувані апаратні платформи (контролери польотів, транспортні засоби, планери, системи телеметрії, системи контролю RC).
...
- [Розділ Політ](flying/README.md) навчає польоту, в тому числі де і як безпечно літати, і як налагоджувати проблеми зі зброєю та польотом. Він також надає детальну інформацію про режими польоту.
...
- Оберіть кадр — [Airframe Builds](airframes/README.md) зі списком підтримуваних кадрів і ознайомтеся з докладними вказівками створення підмножини транспортних засобів.
- Виберіть контролер польоту - див. [Початок > Польоти контролери](getting_started/flight_controller_selection.md) and [Автопілот апаратного](flight_controller/README.md).
- [Збірка](assembly/README.md) пояснює, як підключити важливі периферійні пристрої до вашого автопілота.
- [Базова конфігурація](config/README.md) показує, як оновити прошивку і налаштувати її з параметрами, які відповідають вашому розташуванню. Цей розділ також пояснює, як калібрувати основні датчики (компас, gyro/IMU, швидкість польоту тощо), а також налаштувати ваш дистанційний контроль та функції безпеки.
...
- [Вантажне навантаження](payloads/README.md) описує як інтегрувати корисні навантаження
...
## Повідомлення по помилки та задачі
