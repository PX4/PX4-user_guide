...
| Thone<br>![PMW3901 Thoneflow Notch][3] | Alientek (also has an arrow indicating the front!)<br>![PMW3901 Alientek Notch][4] |
...
This distance sensor is the VL53L0x ToF sensor from STMicroelectronics. The sensor range is minimal (2 metres) and will be reduced when flying in the sunlight. The board also incorporates a distance sensor [wired to the Pixhawk I2C port](#i2c-wiring). We therefore highly recommend that you use an [external distance sensor](#external-rangefinders).
...
In addition, you must also set the parameter [SENS_TFLOW_CFG](../advanced_config/parameter_reference.md#SENS_TFLOW_CFG) to the value of the UART port you connected (e.g. if the sensor is connected to `TELEM 2` then set `SENS_TFLOW_CFG=102`. It can be any unused port, e.g.: `TELEM2`, `TELEM3`, `GPS2` etc. For more information see [Serial Port Configuration](../peripherals/serial_configuration.md).
