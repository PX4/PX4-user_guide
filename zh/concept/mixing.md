...
特定的控制器发送一个特定的归一化的力或力矩指令（缩放至 -1..+1 ）给混控器，混控器则相应地去设置每个单独的执行器。 The output driver (e.g. UART, CAN or PWM) then scales it to the actuators native units, e.g. a PWM value of 1300.
