...
The whole hardware setup is shown in the figure below. It consists of the following parts: It consists of the following parts:
* Odroid C1
* Logitech 摄像头 C920
...
Configure DHCP. Configure DHCP. Edit the file `/etc/udhcpd.conf`
...
You will need to give the Onboard Computer a static IP address. You will need to give the Onboard Computer a static IP address. Edit the file `/etc/network/interfaces` and replace the line `iface wlan0 inet dhcp` (or `iface wlan0 inet manual`) to:
...
Disable the original (WiFi Client) auto configuration. Disable the original (WiFi Client) auto configuration. Change the lines (they probably will not be all next to each other or may not even be there at all):
...
```
Your Onboard Computer should now be hosting a wireless hotspot. Your Onboard Computer should now be hosting a wireless hotspot. To get the hotspot to start on boot, run these additional commands:
