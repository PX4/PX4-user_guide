...
| <a id="key_automatic"></a>[<img src="../../assets/site/automatic_mode.svg" title="Automatic mode" width="30px" />](#key_automatic)                                                                                                                                                                                                                                     | 自动模式. 除非切换模式，否则 RC 控制被默认失能。 Local position estimate required. |
...
当摇杆被释放/回中时，机体将恢复水平和保持当前的*高度*。 You can use it to quickly slow down the vehicle to a safe speed. You can also use it to get more precision from stick input, in particular when flying close to obstacles, or to comply with regulations such as [EASA's low-speed mode/function](https://www.easa.europa.eu/en/light/topics/flying-drones-close-people).
...
:::tip PX4 GCS 叫做[QGroundControl](https://docs.qgroundcontrol.com/master/en/). _QGroundControl_ is the same application we use for [configuring PX4](../config/README.md).
:::
...
_Mission mode_<!-- \[Mission mode\](../flight_modes/mission.md) -->使飞行器执行已上传到飞行控制器的预定义的自主[任务](../flying/missions.md)（飞行计划）。 The mission is typically created and uploaded with a Ground Control Station (GCS) application.
